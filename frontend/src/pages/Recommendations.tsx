...
export const Recommendations: React.FC = () => {
  ...
  const handleStatusChange = async (recId: number, status: string, actionLabel: string) => {
    ...
    try {
      ...
      showToast({
        type: 'critical',
        title: 'Action Failed',
        message: err.message || `Unable to ${actionLabel.toLowerCase()}. Please try again.`
      });
    } finally {
      ...
      {recommendations.length === 0 ? (
        ...
        <div className="space-y-4">
          {recommendations.map((rec) => {
            const isBusy = actionInProgress === rec.recommendation_id;
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between pt-3 border-t border-slate-100 text-xs gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {rec.project_id && (
                      <Link
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-btn text-xs transition-colors shadow-2xs"
                      >
                        <Bot className="w-3.5 h-3.5 text-blue-600" /> Explain Rationale
                      </Link>
                    )}
                    ...
                    {rec.status === 'PENDING' && (
                      <>
                        <button
                          disabled={isBusy}
                          onClick={() => handleStatusChange(rec.recommendation_id, 'REJECTED', 'Dismiss')}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-btn text-xs transition-colors cursor-pointer"
                        >
                          <XCircle className="w-3.5 h-3.5" /> Dismiss
                        </button>
                        <button
                          disabled={isBusy}
                          onClick={() => handleStatusChange(rec.recommendation_id, 'ACCEPTED', 'Accept Intervention')}
                          className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-btn text-xs transition-colors cursor-pointer shadow-xs"
                        >
                          <Check className="w-3.5 h-3.5 stroke-[2.5]" /> Accept Intervention
                        </button>
                      </>
                    )}

                    {/* Accepted State -> Mark as Implemented / Complete */}
                    {rec.status === 'ACCEPTED' && (
                      <button
                        disabled={isBusy}
                        onClick={() => handleStatusChange(rec.recommendation_id, 'IMPLEMENTED', 'Mark Completed')}
                        className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-btn text-xs transition-colors cursor-pointer shadow-xs"
                      >
                    ...
                    {rec.status === 'IMPLEMENTED' && (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 font-bold rounded-btn text-xs border border-emerald-200">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Completed &amp; Verified
                      </span>
                    )}

                    {/* Rejected State -> Allow Re-open */}
                    {rec.status === 'REJECTED' && (
                      <button
                        disabled={isBusy}
                        onClick={() => handleStatusChange(rec.recommendation_id, 'PENDING', 'Re-open')}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-btn text-xs transition-colors cursor-pointer"
                      >
